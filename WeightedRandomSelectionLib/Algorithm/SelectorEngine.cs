...
namespace WeightedRandomSelectorLib.Algorithm
{
    /// <summary>
    ...
    internal class SelectorEngine<T>
    {
        ...
        protected int RollRNG(List<WeightedItem<T>> items)
        {
            ...
            int maxWeight = _selector.Options.HasFlag(SelectorOptions.AllowDuplicates) ? _selector.TotalCumulativeWeight + 1 : (int)items.Sum(i => i.Weight * _selector.IntegerFactor) + 1;
        ...
        {
            List<WeightedItem<T>> items = _selector.Items;
        ...
        internal List<T> SelectMulti(int count)
        {
            ...
            if (!_selector.Options.HasFlag(SelectorOptions.AllowDuplicates))
            {
                ...
                List<WeightedItem<T>> items = new List<WeightedItem<T>>(_selector.Items);
                List<int> weights = new List<int>(_selector.CumulativeWeightsList);
            ...
            else
            {
                for (int i = 0; i < count; i++)
                {
                    resultList.Add(ExecuteSelect(_selector.Items).Value);
                }
            }
